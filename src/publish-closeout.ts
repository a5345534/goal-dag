 ...
 */
export function runPublishCloseout(
  options: PublishCloseoutOptions,
  ...
  try {
    ...
    const behindCount = parseInt(
      git(["rev-list", "--count", `${branch}..${branch}@{upstream}`], cwd).trim(),
      10,
    );
    if (behindCount > 0) {
  ...
  const commitMessage =
    options.commitMessage ?? autoCommitMessage(options.ownedPaths, remote, branch);
  ...
  try {
    if (options.dryRun) {
      process.stderr.write(
        `[dry-run] Would push: git push ${remote} HEAD:refs/heads/${branch}\n`,
      );
    } else {
...
function git(args: string[], cwd: string, execOptions?: ExecOptions): string {
...
function isReadOnlyCommand(args: string[]): boolean {
  ...
  if (readOnlyCmds.includes(cmd)) return true;
  // fetch reads from remote but does not push local changes
  if (cmd === "fetch") return true;
  return false;
}
...
function normalizeOwnedPaths(
  owned: OwnedOutputPaths,
  cwd: string,
): string[] {
  const paths: string[] = [];
...
interface StatusScanResult {
  /** Owned paths that are dirty (modified, added, or untracked) */
  ownedDirty: string[];
  /** Owned paths that have no changes */
  ownedUnchanged: string[];
  /** Dirty paths that are not in the owned set */
  unrelatedDirty: string[];
  /** Paths whose ownership cannot be determined */
 ...
 */
function scanGitStatus(cwd: string, ownedPaths: string[]): StatusScanResult {
  const ownedSet = new Set(ownedPaths.map((p) => normalize(p)));
  const ownedDirty: string[] = [];
  const ownedUnchanged: string[] = [];
  const unrelatedDirty: string[] = [];
  ...
  for (const ownedPath of ownedPaths) {
    try {
      ...
      const untracked = git(
        ["ls-files", "--others", "--exclude-standard", "--", ownedPath],
        cwd,
      );
...
function extractStatusPath(line: string): string | undefined {
...
function isOwnedPath(normalizedPath: string, ownedPaths: string[]): boolean {
  return ownedPaths.some((owned) => {
    ...
    if (normalizedPath === normalizedOwned) return true;
    // Match if owned path is a directory and path is inside it
    if (normalizedPath.startsWith(normalizedOwned + "/")) return true;
