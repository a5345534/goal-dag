...
test("buildGoalDagFromSpecFile rejects missing OpenSpec validators excluded by allowed paths", () => {
  ...
  try {
    ...
    assert.throws(
      () => buildGoalDagFromSpecFile(specPath, outPath, { validationCwd: dir }),
      /requires missing OpenSpec change.*validation\.allowedPaths excludes it/,
    );
  } finally {
...
test("buildGoalDagFromSpec rejects unsupported qualityProfiles through the runtime parser", () => {
  assert.throws(
    () => buildGoalDagFromSpec({
      objective: "x",
      defaults: { qualityProfiles: ["unsupported-profile" as never] },
      nodes: [{ id: "a", objective: "a" }],
    }),
    /quality profile/,
  );
...
test("valid requiredEvidence token build round-trip through serialize and re-validate", () => {
  const tokens: NonNullable<
    GoalDagSpec["nodes"][number]["validation"]
  >["requiredEvidence"] = [
    "validators-ran",
    "locked-artifacts-unchanged",
    "implementation-diff-present",
  ...
  const document = buildGoalDagFromSpec(spec);
  assert.deepEqual(
    document.nodes[0]?.validation?.requiredEvidence,
    tokens,
    "all supported tokens must survive the builder",
  );
  // Serialize → re-validate round-trip
  const json = serializeGoalDagDocument(document);
  const reparsed = validateGoalDagJson(json);
  assert.deepEqual(
    reparsed.nodes[0]?.validation?.requiredEvidence,
    tokens,
    "round-trip through JSON must preserve every token",
  );
  ...
  for (const token of tokens) {
    assert.match(
      ...
      `token ${token} must appear in serialized DAG`,
    );
  }
});
...
test("invalid prose evidence fails during buildGoalDagFromSpecFile before any DAG file is written", () => {
  ...
  try {
    ...
    assert.throws(
      () => buildGoalDagFromSpecFile(specPath, outPath),
      /unsupported value/,
      "must reject prose evidence before writing DAG",
    );
    ...
    assert.equal(
      fileWritten,
      false,
      "DAG file must not be written when evidence validation fails",
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test("producer schema requiredEvidence enum is consistent with builder supported tokens", () => {
  const schema = JSON.parse(
    readFileSync(
      resolve(REPO_ROOT, "schemas", "goal-dag-spec.schema.json"),
      "utf8",
    ),
  );
  const enumValues: string[] =
    schema.$defs?.requiredEvidence?.items?.enum;
  ...
  assert.deepEqual(
    [...enumValues].sort(),
    [...expected].sort(),
    "producer schema enum must match the builder's supported evidence tokens",
  );
  ...
  for (const token of expected) {
    ...
    assert.deepEqual(
      document.nodes[0]?.validation?.requiredEvidence,
      [token],
      `token ${token} must be accepted by the builder`,
    );
  }
  ...
  for (const token of enumValues) {
    ...
    assert.deepEqual(
      document.nodes[0]?.validation?.requiredEvidence,
      [token],
      `schema enum token ${token} must be accepted by the builder`,
    );
  }
});
...
test("buildGoalDagPlanningTrace preserves acceptanceCriteria and evidence exactly", () => {
  ...
  assert.deepEqual(
    trace.nodeQuality.find((n) => n.nodeId === "alpha")?.acceptanceCriteria,
    ["Must compile without errors", "Must pass type-check"],
  );
  assert.deepEqual(
    trace.nodeQuality.find((n) => n.nodeId === "beta")?.acceptanceCriteria,
    ["Must integrate with alpha"],
  );

  // decompositionRationale preserved
  assert.equal(
    trace.nodeQuality.find((n) => n.nodeId === "alpha")?.decompositionRationale,
  ...
  const autoEv = trace.evidence.find(
    (e) => e.quote === "Beta follows alpha per the architecture decision record",
  );
  ...
  assert.ok(
    (trace.transitions.find((t) => t.nodeId === "alpha")?.evidence ?? []).includes(
      "ev-proof",
    ),
  );
  assert.ok(
    (trace.transitions.find((t) => t.nodeId === "beta")?.evidence ?? []).includes(
      autoEv?.id ?? "",
    ),
  );
...
test("runtime DAG output individually strips each trace-only field while trace preserves them", () => {
  ...
  assert.equal(
    trace.modelAssignments[0]?.reason,
  ...
  assert.equal(
    trace.nodeQuality[0]?.decompositionRationale,
    "Single-node implementation scope",
  );
...
test("final-verification fixture maps validators evidence and trace correctly", () => {
  ...
  assert.deepEqual(
    document.defaults?.validators,
    ["npm ci && npm test"],
    "default validators must survive in runtime DAG",
  );
  assert.deepEqual(
    document.nodes[0]?.validators,
    undefined,
    "write-spec inherits no validators",
  );
  assert.deepEqual(
    document.nodes[1]?.validators,
    ["npm ci && npm test", "npm run lint"],
    "implement-x must carry its own validators",
  );
  assert.deepEqual(
    document.nodes[2]?.validators,
    ["npm ci && npm run audit"],
    "review-x must carry its own validators",
  );
  ...
  assert.deepEqual(
    document.nodes[2]?.validation?.requiredEvidence,
    ["validators-ran", "audit-report-present"],
  );
  assert.deepEqual(
    document.nodes[2]?.validation?.allowedPaths,
    ["src/feature-x/**"],
  );
  assert.deepEqual(
    document.nodes[2]?.validation?.forbiddenPaths,
    ["infra/**", "secrets/**"],
  );
  ...
  assert.equal(
    ...
    3,
    "trace must collect 3 evidence items",
  );
  ...
  // Dependency review
  assert.match(
    trace.dependencyReview[1]?.whyNotParallel ?? "",
    /Depends on write-spec/,
  );
  assert.match(
    trace.dependencyReview[2]?.whyNotParallel ?? "",
    /Depends on implement-x/,
  );

  // Model assignments
  assert.equal(
    trace.modelAssignments[0]?.modelClass,
    "implementation",
  );
  assert.equal(
    trace.modelAssignments[1]?.modelClass,
    "implementation",
  );
  assert.equal(
    trace.modelAssignments[2]?.modelClass,
    "strict-reviewer",
  );
  ...
  assert.equal(
    trace.nodeQuality[2]?.decompositionRationale,
    "Single review gate for feature X",
  );

  // No warnings expected for a well-formed fixture
  const fixtureWarnings = trace.warnings.filter(
    (w) => !w.includes("No acceptance handle"),
  );
  assert.equal(
    fixtureWarnings.length,
    0,
    `unexpected trace warnings: ${fixtureWarnings.join("; ")}`,
  );
...
test("DAG JSON boundary purity: contains no concrete provider/model IDs", () => {
  ...
  assert.doesNotMatch(
    json,
    concreteModelPattern,
    "serialized DAG JSON must not contain concrete provider/model IDs",
  );
  ...
  for (const mc of foundModelClasses) {
    assert.match(
      mc,
      /^(controller|implementation|strict-reviewer|evidence-collector|value-judge|spec-writer)/,
      `modelClass "${mc}" must be an abstract class from the catalog, not a concrete provider/model id`,
    );
  }
});
...
test("DAG JSON boundary purity: modelScenario routes to modelClass only", () => {
  ...
  assert.doesNotMatch(
    json,
    /"scenarios":\s*\{[^}]*"model":\s*"/s,
    "modelRouting.scenarios must use modelClass, not concrete model",
  );
  ...
  for (const node of document.nodes) {
    if (node.modelScenario) {
      ...
      assert.ok(
        scenario,
        `node ${node.id} modelScenario "${node.modelScenario}" must exist in scenarios`,
      );
      assert.ok(
        scenario.modelClass,
        `scenario ${node.modelScenario} must have an abstract modelClass`,
      );
    }
  }
});
...
test("DAG JSON boundary purity: rejects concrete model fields at every level", () => {
  ...
  assert.throws(
    ...
    /model is unsupported; use modelClass/,
    "must reject scenario-level concrete model",
  );
  ...
  assert.throws(
    ...
    /model is unsupported; use modelClass/,
    "must reject mixed model + modelClass in scenario",
  );
  ...
  assert.equal(
    bareModelKeys.length,
    0,
    `serialized DAG must contain zero bare "model" keys; found: ${bareModelKeys.join(", ")}`,
  );
