...
test("buildGoalDagFromSpec accepts implementation-discipline quality profile", () => {
    ...
    assert.deepEqual(document.defaults?.qualityProfiles, ["implementation-discipline"], "defaults.qualityProfiles must preserve implementation-discipline");
    assert.equal(document.nodes[0]?.qualityProfiles, undefined, "node a inherits defaults");
    assert.deepEqual(document.nodes[1]?.qualityProfiles, ["implementation-discipline", "test-driven-change"], "node b carries its own qualityProfiles with implementation-discipline");
    // Round-trip through serialize + validate
    const parsed = validateGoalDagJson(serializeGoalDagDocument(document));
    assert.deepEqual(parsed.defaults?.qualityProfiles, ["implementation-discipline"], "implementation-discipline survives round-trip in defaults");
    assert.deepEqual(parsed.nodes[1]?.qualityProfiles, ["implementation-discipline", "test-driven-change"], "implementation-discipline survives round-trip on node");
});
