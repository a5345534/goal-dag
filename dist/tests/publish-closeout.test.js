...
import { runPublishCloseout, } from "../index.js";
...
test("runPublishCloseout returns published after commit and push", () => {
    ...
    try {
        ...
        assert.ok(result.diagnostics.some((d) => d.code === "remote-verified") ||
...
test("runPublishCloseout blocks on unrelated dirty files", () => {
    ...
    try {
        ...
        assert.ok(result.diagnostics.some((d) => d.code === "unrelated-dirty-file"), "should have unrelated-dirty-file diagnostic");
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout blocks on detached HEAD", () => {
    ...
    try {
        ...
        assert.equal(result.mode, "blocked");
        assert.ok(result.diagnostics.some((d) => d.code === "detached-head"));
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout blocks on missing upstream", () => {
    ...
    try {
        ...
        assert.equal(result.mode, "blocked");
        assert.ok(result.diagnostics.some((d) => d.code === "missing-upstream"));
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout returns non_published mode", () => {
    ...
    try {
        ...
        assert.ok(result.diagnostics.some((d) => d.code === "np-mode-active"));
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout blocks on missing remote", () => {
    ...
    try {
        ...
        assert.equal(result.mode, "blocked");
        assert.ok(result.diagnostics.some((d) => d.code === "missing-remote" || d.code === "missing-upstream"));
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout blocks on branch divergence", () => {
    ...
    try {
        ...
        assert.equal(result.mode, "blocked");
        assert.ok(result.diagnostics.some((d) => d.code === "branch-diverged"), "should detect branch divergence");
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout produces diagnostics for every severity level", () => {
    ...
    try {
        ...
        assert.ok(result.diagnostics.some((d) => d.severity === "blocker"));
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout skips owned file that has no changes", () => {
    ...
    try {
        ...
        assert.equal(result.mode, "no_changes");
        assert.ok(result.diagnostics.some((d) => d.code === "no-changes"));
    }
    finally {
        cleanupRepo(repo);
    }
...
test("runPublishCloseout blocks when worktree is dirty after committing owned files and unrelated dirt remains", () => {
    ...
    try {
        ...
        assert.equal(result.mode, "blocked");
        assert.ok(result.diagnostics.some((d) => d.code === "unrelated-dirty-file"), "unrelated dirty file should block");
    }
    finally {
        cleanupRepo(repo);
    }
...
test("build-dag CLI rejects closeout when owned paths cannot be committed", () => {
    ...
    try {
        ...
        try {
            ...
            assert.ok((err.stderr ?? "").includes("blocked") || (err.stdout ?? "").includes("blocked"), `Should output blocked state, got stderr: ${err.stderr}, stdout: ${err.stdout}`);
        }
    }
    finally {
        cleanupRepo(repo);
...
test("runPublishCloseout includes remote verification for GitHub-like remote", () => {
    ...
    try {
        ...
        assert.ok(result.diagnostics.some((d) => d.code === "remote-verification-skipped"));
    }
    finally {
        cleanupRepo(repo);
    }
});
//# sourceMappingURL=publish-closeout.test.js.map